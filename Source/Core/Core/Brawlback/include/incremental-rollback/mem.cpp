...
void ExcludeMem(void* ptr, size_t size)
{
  ...
  exl.excludeGap = boost::icl::construct<boost::icl::discrete_interval<uintptr_t>>(
      ptr_addr, ptr_addr + size, boost::icl::interval_bounds::closed());
                    ...
                    u64& pageCount)
{
  ...
  auto end_pte =
      reinterpret_cast<uintptr_t>(Common::GetPageAddress(base + baseSize, Common::PageSize()));
  ...
  while (base_pte <= end_pte)
  {
    if (memory.IsPageDirty(base_pte))
    {
      ...
      auto addr = memory.GetDirtyPages()[base_pte].fastmem_address;
...
bool GetAndResetWrittenPages(std::vector<uintptr_t>& changedPageAddresses, u64 maxEntries)
{
  ...
  {
    // on input to GetWriteWatch this is the maximum number of possible changed pages in this
    // allocation on output, this is the number of page addresses that have been changed
    ...
    if (result != 0 || pageCount > maxEntries || changedPageAddresses.size() > maxEntries)
    {
      ...
      if (result == 2 || result == 3)
      {
        ...
        ERROR_LOG_FMT(
            BRAWLBACK,
            "WRITTEN PAGE WRITE FAILED DUE TO A FAILURE ({}) TO WRITE PROTECT. THE REASON IS: {}\n",
            result, dw);
      }
      return false;
    }
