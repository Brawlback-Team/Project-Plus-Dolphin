...
bool SendPacket(ENetPeer* socket, const sf::Packet& packet, u8 channel_id, _ENetPacketFlag flag)
{
  if (!socket)
  {
  ...
  ENetPacket* epac = enet_packet_create(packet.getData(), packet.getDataSize(), flag);
  if (!epac)
  {
